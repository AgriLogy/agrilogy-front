...
# # --- JSON File ---
# json_file_path = '/app/requests.json'
...
#         logging.warning(f"❌ User with username '{username}' does not exist. Skipping.")
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.StreamHandler()  # This will log to console as well (optional)
    ]
)

# Log the current time every time the script is run
logging.info("Logging this message every minute at " + str(datetime.now()))
